...
ALTER TABLE sensores ADD COLUMN estado BOOL DEFAULT TRUE;
ALTER TABLE tipo_sensores ADD COLUMN estado BOOL DEFAULT TRUE;

DELIMITER $$

CREATE TRIGGER validar_estado_sensor
BEFORE UPDATE ON sensores
FOR EACH ROW
BEGIN
  DECLARE tipo_estado BOOLEAN;

  SELECT estado INTO tipo_estado
  FROM tipo_sensores
  WHERE id_tipo = NEW.id_tipo_sensor;

  IF tipo_estado = FALSE AND NEW.estado = TRUE THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'No se puede activar un sensor de un tipo inactivo';
  END IF;
END$$
